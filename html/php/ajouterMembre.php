...
if (isset($_POST['login']) && isset($_POST['pass'])) {
	...
	// Check longueur minimum du mot de passe
	} else if (strlen($_POST['pass']) < 8) {
		$erreur = "<h3>Le mot de passe doit contenir au moins 8 caractères</h3>";
	} else {
		$estAdmin = ($_POST['estAdmin']  ? 1 : 0);
		$actif = ($_POST['actif'] ? 1 : 0);
		
		// Hash password
		$userPasswordHash = password_hash($_POST['pass'], PASSWORD_DEFAULT);
 
		$ret = ConnexionDB::ajouterMembre($_POST['login'], $userPasswordHash, $actif, $estAdmin);
		if(!$ret) {
