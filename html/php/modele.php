...
class ConnexionDB {
    ...
    private static function connexion() {
        if ($dbPDO = new PDO('sqlite:/var/www/databases/database.sqlite')) {
            return $dbPDO;
        } else {
            return NULL;
        }
    ...
    public static function connexionMembre($login) {

        $dbClient = self::connexion();

        $stmt = $dbClient->prepare("select id_membre, login, actif, estAdmin, pass from membres where login=? limit 1");
        ...
        if (count($result) > 0) {
            if ($result['actif'] == 1) {
                ...
                $membre = new Membre($result['id_membre'], $result['login'], $result['actif'], $result['estAdmin']);
		$membre->setPass($result['pass']);
		return $membre;
            }
        }
        $db_client = NULL;
